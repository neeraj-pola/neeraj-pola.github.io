...
For gradient we differentiate the divergence for every time step t, so except the t<sup>th</sup>  term, every other time step’s gradient will be zero.
...
![_config.yml]({{ site.baseurl }}/images/CTC/constraint_rules.png)
...
Here, as constrained we initialize with top left node i.e., y^b~0.
